    ...
    <Container>
      ...
      <div className="my-7 flex items-center flex-wrap gap-4">
        {popularSearchItems?.map(({ title, link }) => (
          <Link
            key={title}
            to={`/category/${link}`}
            className="border border-gray-300 px-8 py-3 rounded-full capitalize font-medium hover:bg-black hover:text-white transition-colors duration-200"
          >
            {title}
          </Link>
        ))}
      </div>
      <div className="w-full py-5 md:py-0 my-12 bg-gray-100 rounded-lg flex items-center justify-between overflow-hidden">
        <img
          src={discountImgOne}
      ...
      <div className="mt-7">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 mt-7">
          <div className="border border-r-1 border-gray-300 flex items-center justify-center px-6 py-2 cursor-pointer group">
            <img
              src={brandOne}
              alt="brandOne"
              className="w-36 h-auto group-hover:opacity-50 transition-opacity duration-200"
            />
          </div>
